...
@app.get("/")
async def root():
    return {"message": "Excel Trace Agent API", "version": "1.0.0"}
...
async def upload(
    ...
    """
    Upload Excel file for processing. Requires authentication.
    """

    user_id = current_user.username  # username is user_id
...
async def get_job_status(
    ...
    """
    Get the current status of a job. Requires authentication.
    """
...
async def chat(
    ...
    """
    Chat interface for asking questions or requesting transformations. Requires authentication.
    """
...
async def get_download_templates(
    ...
    """
    Get downloadable template files for a job. Requires authentication.
    """
    from app.download_endpoint import get_template_downloads
    ...
    try:
        ...
        if response['Item']['user_id'] != current_user.username:
            raise HTTPException(status_code=403, detail="Access denied")
...
async def request_transformation(
    ...
    """
    Request a transformation. Requires authentication.
    """
    ...
    print(f"✅ Message sent successfully for job {job_id}")
    return {
...
async def get_templates(
    ...
    """
    Get all generated templates for a job. Requires authentication.
    """
...
async def health_check():
    ...
    return {"status": "healthy", "service": "excel-trace-agent"}